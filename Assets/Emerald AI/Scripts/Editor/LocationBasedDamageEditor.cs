...
namespace EmeraldAI.Utility
{
    ...
    public class LocationBasedDamageEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                Transform ParentSource = self.transform.parent;

                //Get the child colliders within the Emerald AI object.
                if (ParentSource == null)
                {
                    ...
                    foreach (Collider C in m_Colliders)
                    {
                        if (C != null)
                        {
                            ...
                            if (!LocationBasedDamage.LocationBasedDamageClass.Contains(self.ColliderList, lbdc) && C.gameObject != self.gameObject && C.GetComponent<Rigidbody>() != null)
                            {
                ...
                //Get the child colliders within the parent of the Emerald AI object (Puppet Master Support).
                else
                {
                    var m_Colliders = ParentSource.GetComponentsInChildren<Collider>();
                    ...
                    foreach (Collider C in m_Colliders)
                    {
                        ...
                        {
                            LocationBasedDamage.LocationBasedDamageClass lbdc = new LocationBasedDamage.LocationBasedDamageClass(C, 1);
                            if (!LocationBasedDamage.LocationBasedDamageClass.Contains(self.ColliderList, lbdc) && C.gameObject != self.gameObject && C.gameObject != ParentSource.gameObject && C.GetComponent<Rigidbody>() != null)
                            {
                                self.ColliderList.Add(lbdc);
                            }
                        }
